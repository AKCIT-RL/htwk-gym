"""Deploy the Kicking Bikinha (toe-kick) policy on the real Booster T1 robot.
...
Usage:
    python deploy_kicking_bikinha.py \\
        --config Kicking_Bikinha.yaml --net 127.0.0.1 \\
...
from utils.policy_kicking_bikinha import PolicyKickingBikinha
...
class KickingController:
    def __init__(self, cfg_file, ball_x, ball_y, kick_angle_deg, target_z=0.0) -> None:
        ...
        self.policy = PolicyKickingBikinha(cfg=self.cfg)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Deploy Kicking Bikinha on T1 robot.")
    parser.add_argument("--config", required=True, type=str,
                        help="Config file name in configs/ (e.g. Kicking_Bikinha.yaml)")
    ...
    print(f"Starting Kicking Bikinha controller, connecting to {args.net} ...")
