...
class KickingMovementBica(BaseTask):
    ...
    def _create_envs(self):
        ...
        for i in range(self.num_envs):
            ...
            # Create robot actor (pass self_collisions flag like base_walk)
    ...
    def _init_buffers(self):
        ...
        # commands buffer: [cmd_x, cmd_y, cmd_yaw] auto-computed toward the ball
        # (same layout as BaseWalk so transfer-learned weights can be reused)
        self.commands = torch.zeros(self.num_envs, 3, dtype=torch.float, device=self.device)
        # Pre-allocated constants used every step in _compute_observations to
        # avoid per-step allocations (major perf hit at 4096 envs / high freq).
        self._gait_zeros_obs = torch.zeros(self.num_envs, 2, dtype=torch.float, device=self.device)
        ...
        # --- Post-kick hold (Option B) ---
        # When a kick is confirmed (`min_ball_vel_buf > min_ball_vel_s`) we DON'T
        # reset immediately; instead we set `post_kick_hold_buf` (in seconds) and
        # let the robot collect post_kick_stillness reward for `post_kick_hold_s`
        # before resetting (still counted as a success at reset time).
        self.post_kick_hold_buf = torch.zeros(self.num_envs, dtype=torch.float, device=self.device)
        self.kick_succeeded_buf = torch.zeros(self.num_envs, dtype=torch.bool, device=self.device)
    ...
    def _reset_idx(self, env_ids):
        ...
        if len(velocity_before_reset) > 0:
            # Single GPU→CPU sync + extend; cap list size to avoid unbounded growth.
            new_vels = velocity_before_reset.detach().cpu().tolist()
            self.ball_velocities.extend(new_vels)
            if len(self.ball_velocities) > 10000:
                # keep only the most recent 5k samples
                self.ball_velocities = self.ball_velocities[-5000:]

        # Reset robot
        self._reset_dofs(env_ids)
        self._reset_root_states(env_ids)

        # Log kick metrics before resetting buffers
        kick_success_mask = self.kick_detected[env_ids]
        if kick_success_mask.any():
            angular_errors = self.angular_error_at_kick[env_ids][kick_success_mask]
            self.extras["angular_error_at_kick_deg"] = angular_errors.mean()
        ...
        if crossed_mask.any():
            for v in self.ball_ref_y_error[env_ids][crossed_mask].cpu().tolist():
                self._kick_lateral_errors_log.append(v)

        # Sample new kick target direction for reset environments
        ...
        # Reset post-kick hold state and reseed gait_frequency for the new episode
        self.post_kick_hold_buf[env_ids] = 0.0
        self.kick_succeeded_buf[env_ids] = False
        gf_lo, gf_hi = self.cfg["commands"].get("gait_frequency", [1.5, 1.5])
        self.gait_frequency[env_ids] = torch_rand_float(
            gf_lo, gf_hi, (len(env_ids), 1), device=self.device
        ).squeeze(1)
    ...
    def _reset_root_states(self, env_ids):
        # Initialize robot states (index 0) — position always from base_init_state + env_origins
        self.root_states[env_ids, 0, :] = self.base_init_state
        self.root_states[env_ids, 0, :2] += self.env_origins[env_ids, :2]

        # Static: random yaw, small lin_vel noise
        ...
        self.gym.set_actor_root_state_tensor_indexed(
            ...
            gymtorch.unwrap_tensor(actor_indices_to_update),
            num_indices
        )
    ...
    def _resample_kick_target(self, env_ids):
        ...
        angle_range = self.cfg["rewards"].get("kick_target_angle_range_deg", [-45., 45.])
        angles_deg = torch_rand_float(
            angle_range[0], angle_range[1], (len(env_ids), 1), device=self.device
        ).squeeze(1)
    ...
    def get_kick_metrics(self) -> dict:
        ...
        metrics = {}
        if self._kick_angular_errors_log:
            errors = self._kick_angular_errors_log
            metrics["kick/angular_error_deg_mean"] = sum(errors) / len(errors)
    ...
    def step(self, actions):
        ...
        ref_x = self.cfg["rewards"].get("lateral_error_ref_x", 3.0)
        robot_ref_x = self.base_pos[:, 0] + ref_x
        ball_x = self.ball_pos[:, 0]
        crossed_now = (ball_x >= robot_ref_x) & ~self.ball_crossed_ref
        if crossed_now.any():
            target_y = self.kick_target_pos_world[crossed_now, 1]
            # Scale target_y to the ref line (target is at ref_distance, scale linearly)
            ref_dist = self.cfg["rewards"].get("kick_target_ref_distance", 8.0)
            target_y_at_ref = target_y * (ref_x / ref_dist)
            self.ball_ref_y_error[crossed_now] = torch.abs(self.ball_pos[crossed_now, 1] - target_y_at_ref)
            self.ball_crossed_ref[crossed_now] = True
        ...
        # Opção-3 command: aponta para a bola em frame local, proporcional à distância.
        # Quando perto (<approach_cmd_stop_dist), zera para deixar as rewards de chute dominarem.
        self._update_commands_toward_ball()
        ...
        # NOTE: per-step prints removed (they were flushing stdout every sim step
        # and computing np.mean/std/max over an ever-growing python list, which
        # created a major per-step bottleneck — especially with farther ball /
        # longer episodes in KickingMovement).
    ...
    def _check_termination(self):
        ...
        Option B: instead of resetting as soon as a kick succeeds, we enter a
        post-kick hold phase. During the hold the robot keeps physics + the
        post_kick_stillness reward, and only after `post_kick_hold_s` seconds we
        actually reset the env. Falls / out-of-bounds still terminate immediately.
        """
        fell_contact = torch.any(torch.norm(self.contact_forces[:, self.termination_contact_indices, :], dim=-1) > 1.0, dim=1)
        too_fast_root = self.root_states[:, 0, 7:13].square().sum(dim=-1) > self.cfg["rewards"]["terminate_vel"]
        too_low = self.base_pos[:, 2] - self.terrain.terrain_heights(self.base_pos) < self.cfg["rewards"]["terminate_height"]
        self.time_out_buf = self.episode_length_buf > np.ceil(self.cfg["rewards"]["episode_length_s"] / self.dt)

        self.reset_buf = fell_contact | too_fast_root | too_low | self.time_out_buf

        # Mark new successes (kick just confirmed)
        kick_now = (self.min_ball_vel_buf > np.ceil(self.cfg["rewards"]["min_ball_vel_s"] / self.dt)) & ~self.kick_succeeded_buf
        self.kick_succeeded_buf |= kick_now

        # Tick the post-kick hold timer once a kick has been registered
        self.post_kick_hold_buf = torch.where(self.kick_succeeded_buf,
                                              self.post_kick_hold_buf + self.dt,
                                              torch.zeros_like(self.post_kick_hold_buf))

        # End the hold (and reset) when it has lasted long enough
        post_kick_hold_s = float(self.cfg["rewards"].get("post_kick_hold_s", 3.0))
        hold_done = self.kick_succeeded_buf & (self.post_kick_hold_buf >= post_kick_hold_s)
        self.reset_buf |= hold_done

        # Safety nets: ball stuck idle or rolling forever (suppressed during hold)
        not_in_hold = ~self.kick_succeeded_buf
        max_ball_still_time = self.cfg["rewards"].get("max_ball_still_time_s", 4.0)
        self.reset_buf |= (self.time_since_ball_is_still_buf > max_ball_still_time) & not_in_hold
        max_ball_moving_time = self.cfg["rewards"].get("max_ball_moving_time_s", 4.0)
        self.reset_buf |= (self.time_since_ball_is_moving_buf > max_ball_moving_time) & not_in_hold

        self.time_out_buf |= self.episode_length_buf == self.cmd_resample_time

        # Counters (one success counted per kick, falls counted as before)
        self.env_successes += torch.sum(kick_now)
        self.env_falling += torch.sum(too_low)
        self.env_falling += torch.sum(too_fast_root)
    ...
    def _update_commands_toward_ball(self):
        """Option-3 automatic command: steer the robot toward the ball.

        Layout shared with BaseWalk_Extended (slots 6:9 of the 52-obs vector):
          cmd_x, cmd_y  — velocity in robot-local frame toward the ball, clipped to
                          [0, approach_cmd_max_speed] and scaled proportionally to distance.
          cmd_yaw       — gentle yaw correction to face the ball.

        When the robot is closer than `approach_cmd_stop_dist` the command is zeroed
        so the kicking rewards take over without the locomotion command fighting them.
        """
        stop_dist  = self.cfg["rewards"].get("approach_cmd_stop_dist", 0.5)   # [m]
        max_speed  = self.cfg["rewards"].get("approach_cmd_max_speed", 1.0)   # [m/s]
        yaw_gain   = self.cfg["rewards"].get("approach_cmd_yaw_gain",  0.3)   # [rad/s per rad]

        # Ball position in robot-local frame
        to_ball_world = self.ball_pos - self.base_pos                          # [N, 3]
        to_ball_local = quat_rotate_inverse(self.base_quat, to_ball_world)     # [N, 3]

        dist = torch.norm(to_ball_local[:, :2], dim=-1, keepdim=True).clamp(min=1e-6)  # [N,1]

        # Speed proportional to distance, capped at max_speed
        speed = (dist * (max_speed / stop_dist)).clamp(max=max_speed)

        # Unit direction × speed in robot frame
        cmd_xy  = to_ball_local[:, :2] / dist * speed                         # [N, 2]
        cmd_yaw = torch.atan2(to_ball_local[:, 1], to_ball_local[:, 0]).unsqueeze(-1) * yaw_gain  # [N, 1]

        # Zero command when already close — let kicking rewards take over
        near = (dist < stop_dist).float()                                      # [N, 1]
        self.commands[:, 0:2] = cmd_xy  * (1.0 - near)
        self.commands[:, 2:3] = cmd_yaw * (1.0 - near)

    def _compute_observations(self):
        """Computes observations using the unified 53-obs layout shared with BaseWalk_Extended.

        Layout (must match Base_Walk_Extended exactly):
          0: 3  gravity
          3: 6  ang_vel
          6: 9  commands      ← auto-computed by _update_commands_toward_ball (zeros near ball)
          9:11  gait cos/sin  ← zeros (no explicit gait signal needed for kicking)
         11:13  ball_pos_xy   ← real: relative ball XY in robot frame
         13:15  target_dir    ← real: kick target direction [cos θ, sin θ] in robot frame
         15:16  target_z      ← real: target z relative to ball z
         16:17  target_distance ← real: distance from ball to target [m]
         17:29  dof_pos
         29:41  dof_vel
         41:53  actions
        """
        commands_scale = self._commands_scale

        # Ball position relative to robot, in robot-local frame (XY only for obs)
        ...
        target_dir_robot = torch.stack([
            cos_yaw * ball_to_target_norm[:, 0] + sin_yaw * ball_to_target_norm[:, 1],
            -sin_yaw * ball_to_target_norm[:, 0] + cos_yaw * ball_to_target_norm[:, 1],
        ], dim=-1)
        ...
        self.obs_buf = torch.cat(
            (
                ...
                # 6:9  commands (auto-set toward ball by _update_commands_toward_ball)
                self.commands[:, :3] * commands_scale,
                # 9:11  gait cos/sin — zeros (no gait signal in kicking task)
                self._gait_zeros_obs,
                # 11:13  ball_pos_xy in robot frame
                apply_randomization(relative_ball_pos[:, 0:2], self.cfg["noise"].get("ball_pos")) * self.cfg["normalization"]["ball_pos"],
                # 13:15  target direction [cos θ, sin θ]
                target_dir_robot,
                # 15:16  target z relative to ball
                target_z_relative,
                # 16:17  target distance (ball to target, metres)
                self.kick_target_distance.unsqueeze(-1) * self.cfg["normalization"]["target_distance"],
                # 17:29  dof_pos
    ...
    def _reward_tracking_lin_vel_x(self):
        # Tracking of linear velocity commands (x axes) -- follow auto-command toward the ball
        return torch.exp(-torch.square(self.commands[:, 0] - self.filtered_lin_vel[:, 0]) / self.cfg["rewards"]["tracking_sigma"])

    def _reward_tracking_lin_vel_y(self):
        # Tracking of linear velocity commands (y axes) -- follow auto-command toward the ball
        return torch.exp(-torch.square(self.commands[:, 1] - self.filtered_lin_vel[:, 1]) / self.cfg["rewards"]["tracking_sigma"])

    def _reward_tracking_ang_vel(self):
        # Tracking of angular velocity commands (yaw) -- follow auto-command toward the ball
    ...
    def _reward_kicking_foot_approach_ball_stationary(self):
        """Rewards moving the closest foot towards the ball (ported from Kicking)."""
        ...
        return torch.clamp(proximity_value, min=0.0, max=max_reward)

    def _reward_body_alignment_for_kick(self):
        """Rewards aligning the robot's forward direction with the kick target (ported from Kicking)."""
    ...
    def _reward_body_angle(self):
        """Rewards keeping pitch/roll near zero (ported from Kicking)."""
    ...
    def _reward_ball_acceleration(self):
        """Rewards ball acceleration toward the kick target (ported from Kicking)."""
    ...
    def _reward_ball_velocity_target_direction(self):
        ...
        reward = velocity_above_threshold * decay_factor

        max_reward = self.cfg["rewards"].get("max_ball_vel_target_reward", 30.0)
        return torch.clamp(reward, min=0.0, max=max_reward)

    def _reward_post_kick_stillness(self):
        # Active when a kick has been confirmed (Option B hold) OR while the ball
        # has just been moving (decay window). Using kick_succeeded_buf keeps the
        # signal alive even after the ball comes to rest, which is when the
        # robot most needs to stop moving.
        active_hold = self.kick_succeeded_buf.float()
        active_decay = (self.time_since_ball_is_moving_buf > self.cfg["rewards"].get("post_kick_delay_s", 0.5)).float()
        active = torch.clamp(active_hold + active_decay, max=1.0)

        lin_speed = torch.norm(self.base_lin_vel, dim=-1)
        ang_speed = torch.norm(self.base_ang_vel, dim=-1)

        lin_sigma_sq = self.cfg["rewards"].get("post_kick_lin_sigma_sq", 0.05)
        ang_sigma_sq = self.cfg["rewards"].get("post_kick_ang_sigma_sq", 0.05)
        reward = torch.exp(-torch.square(lin_speed) / lin_sigma_sq) * torch.exp(-torch.square(ang_speed) / ang_sigma_sq)
